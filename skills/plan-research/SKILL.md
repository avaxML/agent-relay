...
4. `forum wait TOPIC_ID --timeout 30`, then `forum ingest`. Ingest stores claims and broadcasts them into the other members' inboxes. If claims already agree, skip another round. If jobs disappeared, ingest abandons the round and restores mail; `forum abandon TOPIC_ID` does that explicitly.
5. At most one rebuttal round. Then `forum settle`. Unanimous or majority ballots can agree only when those votes name a `claim_id` whose latest positions match. A split leaves the topic open so the chair can `--claim-id` override (not on an ambiguous id) or close. Treat `consensus.json` as advisory. Verify every surviving citation in source. Never apply a patch because members agreed.
